...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int i = 0; i < N; i++) {
      fg[0] += 700*CppAD::pow(vars[cte_start + i], 2);
      fg[0] += 700*CppAD::pow(vars[epsi_start + i], 2);
...
//
// MPCDriver class definition implementation.
//
MPC::MPC() {}
MPC::~MPC() {}
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  Dvector vars(n_vars);
  for (index = 0; index < n_vars; index++) {
    vars[index] = 0;
  }
  ...
  for (index = 0; index < delta_start; index++) {
  ...
  for (index = delta_start; index < a_start; index++) {
    //vars_lowerbound[i] = -0.436332;
    //vars_upperbound[i] = 0.436332;
    vars_lowerbound[index] = -0.55;
    vars_upperbound[index] = 0.55;
  }

  // Acceleration/decceleration upper and lower limits.
  // NOTE: Feel free to change this to something else.
  for (int index = a_start; index < n_vars; index++) {
    vars_lowerbound[index] = 0.5;
    vars_upperbound[index] = 0.8;
  }
  ...
  for (index = 0; index < n_constraints; index++) {
